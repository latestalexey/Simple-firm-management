...
Процедура ВыполнитьВыбор(ТекущаяОбласть)
	...
	Если ЕдИзм <> Неопределено Тогда
		...
		Сообщить(СтрШаблон(НСтр("ru='В справочнике ""Классификатор единиц измерения"" уже существует элемент с кодом ""%1""! Открыт существующий';uk='У довіднику ""Класифікатор одиниць вимірювання"" вже існує "
"елемент з кодом ""%1""! Відкритий існуючий'"), КодЧисловой));
...
Процедура ИскатьСтрокуВТаблице(Команда)
	...
	Если НайденнаяОбласть = Неопределено Тогда
		...
		Если НайденнаяОбласть = Неопределено Тогда
			Сообщить(НСтр("ru='Строка не найдена';uk='Рядок не знайдено'"), СтатусСообщения.Важное);
			ТекущийЭлемент = Элементы.СтрокаПоиска;
			Возврат;
		КонецЕсли;
	КонецЕсли;
